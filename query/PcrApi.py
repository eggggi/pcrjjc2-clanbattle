...
class PcrApi:
    def __init__(self, accountInfo: Union[dict, int, PcrAccountInfo]):
        ...
        self._UpdateRecord(accountInfo)
    ...
    def OutputName(self) -> str:
        if self._record is not None:
            return f'[{self._record.pcrname}]({self._record.pcrid})'
        if self._pcrClient._viewerId != 0:
            return f'[{self._pcrClient._viewerId}]'
        return f'[{self._pcrClient.biliSdkClient.account}]'
        
    def _UpdateRecord(self, accountInfo) -> None:
        self._record: PcrAccountInfo = accountInfo
    ...
    async def Login(self, force: bool = False) -> None:
        ...
        """
        try:
            await self._pcrClient.LoginAndCheck()
    ...
    async def GetLoadIndexRaw(self) -> dict:
        ...
        # self._record.pcrname = res["user_info"]["user_name"]
        # self._record.save()
        return res
    ...
    async def MultiGiveGift(self, request) -> None:
        await self.CallApi("/room/multi_give_gift", request)
    ...
    async def GetEventInfo(self, event_id: int) -> EventInfoResponse:
        ...
        Raises:
            PcrApiException
        """
        PcrApi.EventInfoResponse.update_forward_refs()
    ...
    async def GetEatPuddingGameInfo(self) -> EatPuddingGameInfoResponse:
        ...
        Raises:
            PcrApiException
        """
        PcrApi.EatPuddingGameInfoResponse.update_forward_refs()
