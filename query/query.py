...
async def VerifyAccount(accountInfo:dict, b_check = True, clean_cache = False) -> None:
    ...
    await client.LoginAndCheck()
    return client.GetAccessKey(), client.GetUID()
...
async def get_chara_formatted(account, chara_id: Union[int, str]):
    '''
    :param chara_id: 四位
    :returns: 详见get_box()
    :raise AssertionError: 该账号没有此角色
    ...
    '''
    chara_id = int(str(chara_id)[:4])
    box = await get_box_formatted(account)
    assert chara_id in box, f'该账号没有角色 {chara.fromid(chara_id).name}'
    return box[chara_id]
...
async def get_clan_id(account):
    ...
    try:
        return home_index["user_clan"]["clan_id"]
    except Exception as e:
        raise Exception("该玩家未加入行会") from e
...
async def get_support_unit_setting(account):
    return await query(account, "/support_unit/get_setting")
